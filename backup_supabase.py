...
Why this exists: until July 2026 there was no backup of the reviews table at all, while
the anon key held a table-wide UPDATE grant on `reviews` (see Section 5 of the technical
reference). A single PATCH against `?status=eq.approved` can rewrite every published row
in one request. Supabase's free tier has no automatic backups, so without this a single
bad request would destroy the entire moderated review corpus with no way back.

Run it before any schema/RLS change, and on a schedule if you can.
...
TABLES = ["reviews", "submissions", "company_comments"]
