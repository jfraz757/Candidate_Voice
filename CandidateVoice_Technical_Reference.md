...
- **Load-contention fix (July 2026):** page used to fire six Supabase requests simultaneously, each paying a cold-connection cost (~530-615ms measured). `loadStats()` no longer downloads the full `employer_name` column (it derived a redundant unique-companies count that duplicated what `loadEmployers()` already fetches for autocomplete — now shares that data, deduped case-insensitively). Count-probe queries select `id` instead of `*`. Startup is now two-phase: grid + stat counts load first, then autocomplete and industry pills load after, on the now-warm connection (~150ms vs ~600ms cold). Grid data now arrives ~445ms vs ~707ms before.
- **Mobile filter UI reworked (July 2026):** filters used to push review cards below the fold on mobile. Outcome and Score filter rows got a `.scroll-row` class — mobile-only horizontal scroll (`overflow-x: auto`, hidden scrollbar) instead of wrapping to multiple lines. The Industry filter (the tallest one, since pills are dynamic/unbounded) became a collapsible toggle button + panel on mobile only (`toggleIndustry()` / `syncIndustryToggle()`, `.industry-open` class), collapsed by default so cards are visible immediately; desktop is unaffected (`.industry-pills-panel` uses `display: contents` there so pills still flow inline like Outcome/Score). **Important:** a previous attempt at this collapsible pattern kept breaking — root cause was `loadIndustryPills()`'s error/empty-state handlers setting inline `style.display = "none"` on `#industry-filter-row`, which collides with any collapse logic that also drives display via inline styles. The fix here uses CSS class toggling exclusively and never touches `style.display` — any future change to this row must keep doing the same or the bug will resurface. Hero/search-section padding and font sizes were also trimmed on mobile as part of this pass.
- **Charset fix (July 2026):** `<meta charset="UTF-8">` must be the very first line in `<head>`, before any `<script>` tags. It was previously placed after the Clarity/Meta Pixel script blocks, past the browser's 1024-byte encoding-detection window, which garbled emoji when loaded locally (masked in production by GitHub Pages' charset HTTP header). Fixed on index.html; other pages with large head scripts should be checked if this recurs.
- **Mobile CTA visibility fix (July 2026):** Clarity showed traffic is ~89% Facebook/Instagram in-app mobile browsers, but the only "Share Your Experience" link lived in `.header-nav`, which is `display: none` at the ≤640px breakpoint and replaced by a hamburger-triggered `.mobile-menu`. Result: 71 of 73 sessions in a 3-day sample landed on the homepage, but only 1 ever reached `submit.html` — the CTA wasn't being found, not being rejected. Fixed by adding two always-visible paths that don't depend on the hamburger: (1) `.btn-hero-cta`, a real button in the hero itself, right after the tagline `<p>` and before `.hero-stats`; (2) `.sticky-cta-bar`, a `position: fixed` bar pinned to the bottom of the viewport, mobile-only (≤640px, same breakpoint as the hamburger), with `env(safe-area-inset-bottom)` padding for iOS home-indicator clearance and a matching `body { padding-bottom: 64px }` so it never covers page content. Both link straight to `submit.html`. Desktop is untouched — the existing `.header-nav` CTA still works there and neither new element is a desktop concern. See Section 12 for the CSS classes. **The hero button was later repurposed — see the next note.**
- **Hero CTA changed from "Share your experience" to "Look up a company" (July 2026, shortly after the above).** `.btn-hero-cta` is now a `<button>` calling `focusSearch()`, which scrolls the employer search into view and focuses it after a 400ms delay (focusing immediately makes mobile browsers open the keyboard and fight the smooth scroll). **Why:** a 62-session Clarity review found every positive engagement signal was someone *reading* — About page, leaderboard, a specific company's reviews — while the homepage asked four separate times for a contribution (sticky bar, header nav, mobile menu, hero) before offering the lookup visitors actually arrive wanting. Median session was **12 seconds with 94% of sessions recording zero clicks**. The contribute ask still exists in three places, and the sticky bottom bar is the better host for it since it catches people *after* they have read a review and recognised their own experience. Being a `<button>` rather than an `<a>` it needs `font-family: inherit` and `cursor: pointer` explicitly, both added.
- **Do not diagnose this funnel from `submit.html` pageviews alone.** `index.html` has an inline submit modal, so a visitor can open the form and abandon it without ever generating a `submit.html` pageview. An earlier analysis concluded "only 1 person ever reached the form" from top-pages data and was wrong for exactly this reason. Use the Clarity "Submit form" smart event or session recordings instead.
...
- **Website field made optional, validation softened (July 2026):** the employer website field lost its `required` asterisk (has a hint instead) since requiring it was blocking submissions. Its favicon-based existence check (Google's favicon endpoint, not real verification) raised its timeout 4s→7s and now shows a neutral "couldn't preview this link" on failure instead of an alarming warning. Missing required fields and submission failures now surface as an inline `.form-error` banner with the offending field marked `.field-invalid`, replacing blocking `alert()` calls — form data stays intact either way. Same alert()→inline pattern should be followed for any future validation added here or in index.html's modal.
...
- **Missing-website lookup (July 2026):** since submit.html's website field is now optional (see above), pending reviews can arrive with no `employer_website`. Pending-tab cards missing it show a "⚠ No Website" badge with a lookup UI — queries Clearbit's autocomplete endpoint (`autocomplete.clearbit.com`, no key required) as-you-type, falls back to a Google search link if no match, and saves the picked/typed value straight to `submissions.employer_website` on select/blur.
...
22. **Zero submissions despite growing social-driven traffic — diagnosed via Microsoft Clarity (July 2026).** Joe was running ads and engaging in comments to drive visits, traffic was up, but submissions stayed at zero. Checked the Clarity dashboard (project `xr6km3ywvh`, see Section 2): 73 sessions / 3 days, 89% via Facebook or Instagram's in-app browser, 1.26 pages/session, 86% avg scroll depth — and critically, **only 1 of 73 sessions ever reached `submit.html`**. That ratio was the tell: this wasn't people reaching the form and abandoning it, it was people never finding the form at all. Cross-referencing against the actual HTML confirmed why: the only "Share Your Experience" link lived inside `.header-nav`, hidden (`display: none`) at the same ≤640px breakpoint that 89% of visitors were on, with no fallback CTA anywhere else on the page. Fix described in Section 7/Section 12 (hero button + sticky bottom bar). **The general lesson: if a conversion funnel looks broken, check Clarity's top-pages list before assuming the form itself is the problem** — a page nobody ever lands on will always show zero conversions, and that's indistinguishable from a broken form unless you check which step of the funnel traffic is actually reaching. Also worth setting up an actual Clarity funnel (homepage → submit.html → form-submitted) instead of inferring drop-off from raw page-view counts — not yet done, listed in Section 16.
...
- **Hero CTA button (`.btn-hero-cta`, added July 2026):** solid orange (`#f5a623`) button in the hero, below the tagline, above the stats row. Same brand orange as `.nav-link.primary`, just a bigger tap target for a section that's visible without scrolling on any breakpoint.
- **Sticky mobile CTA bar (`.sticky-cta-bar`, added July 2026):** `display: none` by default; at ≤640px becomes `position: fixed; bottom: 0`, white background, `#e8eaf0` top border, soft upward box-shadow, containing one full-width orange button. Exists specifically because the hamburger-gated nav CTA was going undiscovered on mobile — see Section 10, note 22.
...
- **Facebook ad performance — reviewed 2026-07-29, and the conclusion was that the page is not the problem.** Two campaigns had run: "CandidateVoice - 88% Stat Card" (149 landing page views, $0.29 each, $42.94) and "Post: We analyzed 544 real hiring experiences" (180 views, $0.26 each, $47.69) — **$90.63 spent, 329 landing page views, 0 submissions.** Both were optimized for **Landing Page Views**, so Meta correctly bought the cheapest people who would load a page, and "will load a page" selects for almost no intent: median session 12 seconds, 94% zero clicks. **Zero conversions was the arithmetic of the objective, not a failure of the site or the ads.** The Meta Pixel's `Lead` event is installed but was never the optimization target, so Meta had no conversion signal to learn from. **Do not simply switch the objective to Conversions** — those campaigns need roughly 50 conversions/week per ad set to exit the learning phase, and starting from zero they under-deliver badly. Fix the funnel until submissions are non-zero, then switch. Three independent signals agree the *data* is the draw rather than the appeal to contribute: the best-performing ad cited 544 real experiences, the strongest on-site engagement was leaderboard → a specific company's reviews, and the strongest above-the-fold asset is "88% ghosted rate across 422 companies".
- **Things checked and cleared during that review, so they don't get re-investigated:** the submit form is not overloaded (20 fields but only 2 required — Employer Name and Outcome — and 7 of 10 dropdowns default to a visible "Skip"); the employer search is above the fold on mobile; hero stats populate rather than showing placeholder dashes; LCP is 1.3s with a 95/100 performance score; zero rage clicks and zero dead clicks. **The page passes every check. The traffic was the problem.**
- **Check the mobile-CTA fix actually moved the needle.** July 2026's hero button + sticky bar (Section 10, note 22) was shipped to fix a near-zero `submit.html` visit rate; check Clarity's top-pages list a few days to a week after traffic accumulates on the new version to confirm `submit.html` sessions and actual submissions went up. If they didn't, the bottleneck is further down the funnel (the form itself, not its discoverability) and needs its own Clarity-driven diagnosis. Setting up a real Clarity funnel (homepage → submit.html → form-submitted) instead of inferring from raw page-view counts would make this check exact rather than approximate — not yet done.
